...
begin
  ...
  update fued_public.game_sessions as gs
  set
    session_status = 'ended',
    ended_at = now(),
    updated_at = now(),
    last_activity_at = now()
  where gs.owner_user_id = v_user_id
    and gs.session_status = 'live';
...
begin
  ...
  update fued_public.session_state as st
  set
...
begin
  ...
  update fued_public.session_board_answers as sba
  set
    is_revealed = true,
    revealed_at = now(),
  ...
  if p_scoring_target = 'team_1' then
    update fued_public.session_state as st
    set
      score_1 = st.score_1 + v_point_value,
      updated_by_user_id = auth.uid(),
      updated_at = now()
    where st.session_id = p_session_id;
    ...
    update fued_public.session_state as st
    set
      score_2 = st.score_2 + v_point_value,
      updated_by_user_id = auth.uid(),
      updated_at = now()
    where st.session_id = p_session_id;
  else
...
begin
  ...
  update fued_public.session_state as st
  set
    strikes_count = least(st.strikes_count + 1, 3),
    updated_by_user_id = auth.uid(),
    updated_at = now()
  where st.session_id = p_session_id;
...
begin
  ...
  update fued_public.session_state as st
  set
    strikes_count = greatest(st.strikes_count - 1, 0),
    updated_by_user_id = auth.uid(),
    updated_at = now()
  where st.session_id = p_session_id;
